...
_PORTAL_PERMS: dict[tuple[str, str], set[str]] = {
    ...
    # Vendors can also buy event tickets (see buy_event_ticket in
    # profile_actions.py) — missing this entry meant _perms_for fell
    # through to ("vendor","*")=set(), so row_actions_allowed was empty
    # and the list rendered with no View/eye button at all.
    ("vendor", "event_ticket_items"): {"view"},
    ("vendor", "*"):              set(),
...
def get_image_url(image_path: str | None, society_id: int | None = None,
    ...
    if '/' not in path and '\\' not in path:
        ...
        elif entity == "society" and pk is not None:
...
def render_list_card(card_id: str, title: str, icon: str,
    ...
    if "new" in allowed:
        ...
        if entity in ("apartments", "vendors", "security", "apartment_users", "assets"):
            header_right.append(dbc.Button(
                [html.I(className="fas fa-file-excel me-1"), "Bulk Enroll"],
...
def render_form_card(card_id: str, title: str, icon: str,
    ...
    if entity == "patrol_location":
        # Interactive map widget for dropping a pin to get lat/lon
        map_html = html.Div(
...
def render_financial_statements_card(
    ...
    def _make_balance_sheet_preview(rows, title):
        """
        Balance Sheet preview, grouped by statement_section (Assets /
        ...
        head = html.Thead(html.Tr([
            html.Th(col, style={"fontSize": "11px"}) for col in ["Account", "Amount"]
        ], style={"fontSize": "11px"}))

        body_rows = []
        current_section = None
        section_total = 0.0
        grand_total_assets = 0.0
        grand_total_liab_equity = 0.0

        def _section_header(name):
            return html.Tr(html.Td(name, colSpan=2, style={
                "fontSize": "11px", "fontWeight": "700", "color": "#fff",
                "background": color, "padding": "6px 8px",
            }))

        def _subtotal_row(name, amount):
            return html.Tr([
                html.Td(name, style={"fontSize": "12px", "fontWeight": "700", "padding": "6px 8px",
                                      "borderTop": "1px solid #ddd"}),
                html.Td(f"₹{amount:,.2f}", style={"fontSize": "12px", "fontWeight": "700", "padding": "6px 8px",
                                                   "borderTop": "1px solid #ddd", "textAlign": "right"}),
            ], style={"background": "#f7f7f7"})

        for r in rows:
            section = r.get("statement_section") or ""
            amount = float(r.get("amount") or 0)
            if section != current_section:
                if current_section is not None:
                    body_rows.append(_subtotal_row(f"Total {current_section}", section_total))
                    if current_section == "Assets":
                        grand_total_assets += section_total
                    else:
                        grand_total_liab_equity += section_total
                current_section = section
                section_total = 0.0
                body_rows.append(_section_header(section))
            body_rows.append(html.Tr([
                html.Td(r.get("account_name") or "", style={"fontSize": "12px", "padding": "6px 8px 6px 20px"}),
                html.Td(f"₹{amount:,.2f}", style={"fontSize": "12px", "padding": "6px 8px", "textAlign": "right"}),
            ]))
        ...
        diff = grand_total_assets - grand_total_liab_equity
        body_rows.append(html.Tr([
            html.Td("Total Assets", style={"fontSize": "12px", "fontWeight": "700", "padding": "8px",
                                            "borderTop": "2px solid #333"}),
            html.Td(f"₹{grand_total_assets:,.2f}", style={"fontSize": "12px", "fontWeight": "700", "padding": "8px",
                                                           "borderTop": "2px solid #333", "textAlign": "right"}),
        ]))
        body_rows.append(html.Tr([
            html.Td("Total Liabilities + Equity", style={"fontSize": "12px", "fontWeight": "700", "padding": "8px"}),
            html.Td(f"₹{grand_total_liab_equity:,.2f}", style={"fontSize": "12px", "fontWeight": "700", "padding": "8px",
                                                                "textAlign": "right"}),
        ]))
        if abs(diff) > 0.01:
            body_rows.append(html.Tr(html.Td(
                f"⚠ Out of balance by ₹{abs(diff):,.2f}", colSpan=2,
                style={"fontSize": "11px", "color": "#c0392b", "fontStyle": "italic", "padding": "6px 8px"},
            )))
...
# ════════════════════════════════════════════════════════════════════════════
# AGREEMENT CARD — society onboarding license agreement, execution page
# ════════════════════════════════════════════════════════════════════════════

def render_agreement_card(society: dict, agreement_record: dict) -> html.Div:
    """
    Auto-shown once, right after a society completes the Setup Wizard
    (submit_setup_wizard), and reachable again later for reprint. Same
    editable-textarea + Print/PDF/Email pattern as render_noc_card (see
    agreement_callbacks.py) — persisted body_text means a later edit to
    the society's profile (secretary changed, address changed) never
    rewrites what was actually agreed to at onboarding.

    Unlike render_noc_card, this isn't scoped to one apartment — it's one
    record per society (society_agreements.society_id is UNIQUE) — and
    carries a two-party execution block (society vs "Authorised
    Signatory, EstateHub, LLC") built into the body text itself, rather
    than the shared letterhead's single-signatory footer.
    """
    color = "#5b3ea6"
    society_nm = society.get("name") or "____"
    society_addr = society.get("address") or "____"
    reg_no = society.get("registration_number") or "____"
    sec_name = society.get("secretary_name") or "____"
    sec_email = society.get("secretary_email") or "____"
    from datetime import date as _date
    today_str = _date.today().strftime("%d %B %Y")
    agreement_no = (agreement_record or {}).get("agreement_no") or "PREVIEW — not yet issued"

    # A reused (already-generated) agreement keeps its original wording —
    # same reasoning as NOC's body_text reuse (see render_noc_card).
    existing_text = (agreement_record or {}).get("body_text") or ""
    if existing_text:
        agreement_text = existing_text
    else:
        agreement_text = (
            "ESTATEHUB SOFTWARE LICENSE AND TERMS OF SERVICE AGREEMENT\n"
            f"Agreement No: {agreement_no}\n"
            f"Date: {today_str}\n\n"
            "IMPORTANT — READ CAREFULLY: This Software License and Terms of "
            "Service Agreement (the \"Agreement\") is a legal agreement "
            f"between {society_nm} (the \"Licensee\") and EstateHub, LLC, "
            "the owner of the EstateHub Society Management Software (the "
            "\"Licensor\"). By configuring, accessing, or using the "
            "EstateHub software (the \"Software\"), Licensee agrees to be "
            "bound by the terms of this Agreement.\n\n"
            "1. GRANT OF LICENSE\n"
            "Subject to the terms and conditions of this Agreement, "
            "Licensor grants Licensee a non-exclusive, non-transferable "
            "license to use the Software for the purpose of managing the "
            "residential or commercial society for which it has been "
            "deployed.\n\n"
            "2. NO LIABILITY\n"
            "To the maximum extent permitted by applicable law, in no "
            "event shall the Licensor, its associates, partners, heirs, "
            "employees, or affiliates be liable for any special, "
            "incidental, indirect, punitive, or consequential damages "
            "whatsoever arising out of or in any way related to the use "
            "of or inability to use the Software. The Software is "
            "provided \"as is\" and \"as available\" with all faults and "
            "without warranty of any kind.\n\n"
            "3. DATA AND COMPLIANCE\n"
            "The Licensee holds all responsibility for the data entered, "
            "managed, and exported from the Software. The Licensor "
            "accepts no liability for non-compliance with local, state, "
            "or federal laws, tax discrepancies, or data loss. The "
            "Licensee is solely responsible for ensuring that accounting, "
            "taxes (including GST and TDS), and compliance reports "
            "generated by the Software are accurate and legally sound.\n\n"
            "4. INDEMNIFICATION\n"
            "The Licensee agrees to indemnify, defend, and hold harmless "
            "the Licensor, its associates, partners, and heirs from any "
            "claims, liabilities, damages, losses, and expenses arising "
            "out of or in any way connected with the Licensee's access to "
            "or use of the Software or violation of this Agreement.\n\n"
            "5. SEVERABILITY\n"
        ...
        if agreement_record and agreement_record.get("id"):
            try:
                db._execute(
                    "UPDATE society_agreements SET body_text=%s WHERE id=%s",
                    (agreement_text, agreement_record["id"]),
                )
            except Exception as e:
                print(f"⚠️  Agreement body_text persist failed: {e}")

    from app.dash_apps.callbacks.print_letterhead import get_letterhead_assets
    ...
    agreement_letterhead_data = {
        ...
        # empty (it collapses cleanly with no assets, same as NOC does for
        # a not-yet-eligible preview) since the two-party execution block
        # is already embedded in the body text above.
        "agreement_no": agreement_no,
    }

    return dbc.Card([
        dcc.Store(id="agreement-letterhead-data", data=agreement_letterhead_data, storage_type="memory"),
        dbc.CardHeader(
            html.Div([
                html.Div(html.I(className="fas fa-file-signature",
                                style={"color": "#fff", "fontSize": "16px"}),
                         style={"width": "38px", "height": "38px", "borderRadius": "10px",
                                "background": f"linear-gradient(135deg,{color},{color}aa)",
                                "display": "flex", "alignItems": "center",
                                "justifyContent": "center", "marginRight": "12px"}),
                html.Div([
                    html.Strong("Society Onboarding Agreement", style={"fontSize": "14px"}),
                    html.Div(f"{society_nm} — {agreement_no}",
                             style={"fontSize": "11px", "color": "#999"}),
                ]),
            ], style={"display": "flex", "alignItems": "center"}),
            style={"padding": "12px 16px",
        ...
        dbc.CardBody([
            dbc.Alert([
                html.I(className="fas fa-check-circle me-2"),
                "Society setup completed — review and print this Agreement for your records.",
            ], color="success", style={"fontSize": "12px", "borderRadius": "10px",
                                       "padding": "8px 14px", "marginBottom": "12px"}),
            dbc.Label("Edit Agreement text below before printing:",
                      style={"fontSize": "11px", "color": "#7d8ea3",
                             "fontWeight": "600", "marginBottom": "4px"}),
            dbc.Textarea(
            ...
            html.Div([
                html.Button(
                    [html.I(className="fas fa-print me-2"), "Print"],
                    id="agreement-btn-print", n_clicks=0,
                    className="btn btn-outline-primary",
                    style={"borderRadius": "10px", "fontWeight": "600"},
                ),
                html.Button(
                    [html.I(className="fas fa-file-pdf me-2"), "Save as PDF"],
                    id="agreement-btn-pdf", n_clicks=0,
                    className="btn btn-outline-danger",
                    style={"borderRadius": "10px", "fontWeight": "600"},
                ),
                html.Button(
                    [html.I(className="fas fa-envelope me-2"), "Email Agreement"],
                    id="agreement-btn-email", n_clicks=0,
                    className="btn btn-outline-info",
                    style={"borderRadius": "10px", "fontWeight": "600"},
                ),
            ], style={"display": "flex", "gap": "10px", "flexWrap": "wrap",
