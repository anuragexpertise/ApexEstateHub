...
Fixed: removed invalid 'from dash.callback_context import callback_context'
       replaced with 'from dash import ctx' (Dash 2.x standard)
"""
...
def _save_upload(contents, filename, subfolder=""):
    ...
    try:
        ...
        return (f"/static/uploads/{subfolder}/{safe_name}"
                if subfolder else f"/static/uploads/{safe_name}")
...
def register_card_catalogue_callbacks(app):
    ...
    def create_society(n, name, email, phone, address, sec_name, sec_phone,
        ...
        if not name:
            return _err("Society name is required"), no_update
    ...
    def save_society_profile(n, name, email, phone, address, sec_name, sec_phone, auth_data):
        if not n:
            return no_update, no_update
        if not name:
            return _err("Society name is required"), no_update
        sid = _sid(auth_data)
        if not sid:
            return _err("No society selected"), no_update
        try:
            _db().execute_query(
                "UPDATE societies SET name=%s,email=%s,phone=%s,address=%s,"
                "secretary_name=%s,secretary_phone=%s WHERE id=%s",
    ...
    def create_entity(n, flat, name, mobile, size, role, email, password,
                      avatar_c, avatar_f, auth_data):
        if not n:
            return no_update, no_update
        ...
        try:
            ...
            user = _db().execute_query(
                ...
                (sid, email, generate_password_hash(password), role), fetch_one=True)
            if user and role == "apartment" and flat:
                apt = _db().execute_query(
                    ...
                    "ON CONFLICT (society_id, flat_number) DO NOTHING RETURNING id",
                    (sid, flat, name, mobile or "", int(size or 0)), fetch_one=True)
                if apt:
                    _db().execute_query(
                        "UPDATE users SET linked_id=%s WHERE id=%s",
                        (apt["id"], user["id"]))
            return _ok(f"Entity '{name or email}' created"), \
                   {"type": "success", "message": "Entity created"}
        except Exception as e:
            return _err(str(e)), no_update
    ...
    def create_account(n, name, tab, header, parent, drcr, bf_amt, bf_type, auth_data):
        if not n:
            return no_update, no_update
        ...
        try:
            _db().execute_query(
                ...
                (sid, role_code, int(entity_id),
                 time_in or datetime.now().isoformat(), time_out or None))
    ...
    # ── 22. Camera clientside callback ───────────────────────────────────
    # (evaluate_pass validation is handled by camera_callbacks.py)
    # We only wire the camera store JS bridge here if camera_callbacks
    # is not registered — so this is a safe no-op duplicate guard.
