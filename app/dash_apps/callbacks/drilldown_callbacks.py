...
def _get_or_create_agreement(db, society_id, created_by):
    ...
    the society profile never rewrites what was actually agreed to at
    onboarding. UNIQUE(society_id) — one agreement per society, never
    reissued/regenerated by this function once created.

    agreement_no format: AGR/<society_id>/<year>/<id> — assigned after
    insert since it embeds the row's own id, same convention as nocs'
    certificate_no.

    No verification QR is generated here (unlike NOC/receipts) — the
    Agreement is an internal onboarding record, not something a third
    party scans to verify; qr_payload is left NULL, reserved for that if
    ever needed later.
    """
    existing = db._execute(
        "SELECT * FROM society_agreements WHERE society_id=%s", (society_id,), fetch_one=True,
    )
    if existing:
        return dict(existing)

    society = db._execute("SELECT * FROM societies WHERE id=%s", (society_id,), fetch_one=True) or {}
    row = db._execute(
    ...
    agreement_id = row["id"]
    from datetime import date
    agreement_no = f"AGR/{society_id}/{date.today().year}/{agreement_id}"
    updated = db._execute(
        "UPDATE society_agreements SET agreement_no=%s WHERE id=%s RETURNING *",
        (agreement_no, agreement_id), fetch_one=True,
    )
    return dict(updated)
