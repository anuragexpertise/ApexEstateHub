...
def create_header(society_name='ApexEstateHub', role='admin',
                  user_email=None, user_avatar=None):
    """Header with glassmorphism design. Hamburger is rendered in layout.py."""

    role_config = {
        'admin':     {'portal': 'Admin Portal',    'color': '#ADD8E6', 'icon': 'fa-user-shield'},
        'apartment': {'portal': 'Owner Portal',    'color': '#90EE90', 'icon': 'fa-home'},
        'vendor':    {'portal': 'Vendor Portal',   'color': '#FFD700', 'icon': 'fa-briefcase'},
        'security':  {'portal': 'Security Portal', 'color': '#F08080', 'icon': 'fa-shield-alt'},
        'master':    {'portal': 'Master Admin',    'color': '#FF6B6B', 'icon': 'fa-crown'},
    }

    cfg         = role_config.get(role, role_config['admin'])
    role_color  = cfg['color']
    portal_name = cfg['portal']
    icon_class  = cfg['icon']

    if not user_avatar:
        safe_name  = (user_email or 'User').split('@')[0]
        hex_color  = role_color.lstrip('#')
        user_avatar = (
            f"https://ui-avatars.com/api/?name={safe_name}"
            f"&background={hex_color}&color=fff&size=128"
        )
    ...
    qr_modal = dbc.Modal(
        [
            ...
            dbc.ModalBody(
                html.Div(
                    [
                        ...
                        html.P('Scan this QR code at society gate for entry',
            ...
            dbc.ModalFooter(
                [
                    dbc.Button('Download QR', id='download-qr-btn',
    ...
    header = html.Div(
        [
            dbc.Container(
                dbc.Row(
                    [
                        # Left: Society name (desktop only)
                        dbc.Col(
                            html.Div(
                                [
                            ...
                            width='auto', className='d-none d-md-flex'
                        ),
                            ...
                            html.Div(
                                [
                                    html.I(className=f'fas {icon_class} me-2',
                                           style={'fontSize': '18px', 'color': role_color}),
                                    html.Span(portal_name,
                            ...
                            dbc.DropdownMenu(
                                [
                                    ...
                                    dbc.DropdownMenuItem(
                                        ...
                                        id='notifications-btn', n_clicks=0
                                    ),
                                    dbc.DropdownMenuItem(
                                        [html.I(className='fas fa-cog me-2'), 'Settings'],
                                        id='settings-btn', n_clicks=0
                                    ),
                                    dbc.DropdownMenuItem(divider=True),
                                    dbc.DropdownMenuItem(
                ...
                fluid=True, style={'padding': '0 16px'}
            ),
            qr_modal,
        ],
        ...
        style={
            ...
            'top': '0',
            'right': '0',
            'left': '250px',            # overridden to 0 on mobile via CSS
            'zIndex': '1000',
