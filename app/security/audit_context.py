...
or `confirmed_by` on writes, since anyone can forge it to impersonate
another user in the audit trail.

This module resolves the acting user from the server-side Flask-Login
...
def get_current_user_id() -> int | None:
    ...
    explicitly whether to fall back to a client-supplied value (and, if
    so, treat that write as NOT audit-trustworthy).
    """
    try:
