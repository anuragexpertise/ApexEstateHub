...
def ensure_master_admin() -> bool:
    """Create master admin if no users exist. Returns True if created."""
    try:
...
def ensure_dummy_data():
    ...
    try:
        ...
        print(f"❌  Error in ensure_dummy_data: {e}")
        import traceback
        traceback.print_exc()


def _seed_dummy():
    """Create dummy society and users."""
    society_id = None
...
def run():
    """Main seeding function called by migrate.py."""
    print()
    print("── Seed Check ────────────────────────────────────────────────────────")
    ensure_master_admin()
    ensure_dummy_data()
